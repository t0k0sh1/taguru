...
- Doc/paper files: named concepts, entities, citations. A decision's rationale (WHY it was made) is its own node with `file_type:"rationale"`, linked to what it explains via a `rationale_for` edge — never folded into another node's attributes. Use `file_type:"rationale"` for concept-like nodes that express reasoning (ideas, principles, mechanisms) and `file_type:"concept"` for named concepts that do not themselves explain a decision. `file_type` MUST be one of exactly these six values: `code`, `document`, `paper`, `image`, `rationale`, `concept`. Any other value is invalid and will be rejected.
