...
| `TAGURU_EMBED_URL` / `_MODEL` / `_API_KEY` | — | Semantic entry tier: an OpenAI-compatible `/embeddings` endpoint, or `local` to run the model in-process (`taguru-code models` lists them; not in the Docker image); unset keeps the entrance purely lexical |
