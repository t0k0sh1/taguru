# 0011. Temporal validity for associations — source-derived valid time on the graph lanes
...
valid time lives, how `since`/`until` reach the graph lanes (`activate`,
`explore`, `recall`, `query`), what a time window does to edge weights, and
what all of it costs in wire contract, on-disk format, and cache terms.
...
answered in §9 rather than silently dropped: bi-temporal modeling, automatic
supersession or forgetting, and temporal *scoring*.
...
**B. Source-derived validity: an edge's temporal position is the effective
time of the sources that attest it.** Chosen; §4. No write-format change of
...
**An association's valid time is derived from its attributing sources'
effective time (`date.or(stored_at)`, the `SourceFilter` rule verbatim).
The graph lanes gain optional `since`/`until` request parameters; a
windowed query sees an edge iff at least one in-window source attests it,
and sees that edge's weight recomputed from its in-window attributions
only. Nothing is written, nothing is invalidated, no format moves.**
...
`Z --CEO--> Y`. An as-of-2023 query sees X alone. An unwindowed query sees
both — and that is correct, because the store's posture ("no silent
rewrites"; every consolidation is proposal-only, ADR issue #421) means the
*current-truth* judgment belongs to the reader, not to a write-path side
effect. Three consequences, fixed here:
...
change CEOs). Valid time reframes the invariant as **"at most one object
at a time"** — a violation is two *concurrently-valid* attestations of
different objects, which is checkable at audit time from source dates with
no write-path read at all, sidestepping the exact hazard that forced the
deferral. This ADR deliberately does not design that check: it belongs to
the cardinality follow-up ADR 0009 §9.2 already demands, with #421's audit
as its natural carrier. What is fixed here is only that the temporal
representation (source-derived, per-attribution) is sufficient input for
it — no edge-carried validity is needed to get there.
...
- #421's staleness/contradiction audit gains a well-posed foundation
  (§6, §7); the cardinality follow-up ADR gains its invariant (§7).
