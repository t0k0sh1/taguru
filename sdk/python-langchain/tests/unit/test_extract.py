...
def test_the_prompt_version_and_wording_track_extract_rs() -> None:
    ...
    (6: #851/#904's document -> segment rename; 5: #812's citation rule;
    4: #852's ground rules), and the wording that earned each bump is in
    the prompt — ports of extract.rs
    ...
    assert "the segment's text alone" in prompt
    assert "Never build a subject or object out of words the segment does not contain" in prompt
