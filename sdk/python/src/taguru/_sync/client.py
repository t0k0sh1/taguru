...
class Taguru:
    ...
    def _ensure_contract(self) -> None:
        ...
        """
        run_contract_probe_once(self._contract_state, self._probe_contract)
    ...
    def _probe_contract(self) -> None:
        ...
        """
        try:
            response = self._send_core("GET", VERSION_PATH, retries=0)
...
class Context:
    ...
    def _post_full(
        ...
        ``unwrap_envelope_full``."""
        return self._client._request_json_full(
            "POST", self._path + suffix, json_body=json_body, retry=retry
        )
    ...
    def explain_resolve_label(
        ...
        *,
        ...
        result = self._post("/resolve_label/explain", body)
        return decode(ResolveExplanation, result)  # type: ignore[no-any-return]
    ...
    def get_schema(self) -> SchemaDocument:
        ...
        request.
        """
        result = self._client._request_json("GET", self._path + "/schema")
    ...
    def add_associations(self, associations: Sequence[AssocOp]) -> AddAssociationsResult:
        ...
        """
        result, issues, schema_violations = self._post_full(
    ...
    def add_associations_batched(
        ...
        *,
        ...
        for chunk in chunk_associations(list(associations), chunk_size, max_chunk_bytes):
            outcome = self.add_associations(chunk)
    ...
    def export_to_file(self, path: str | Path) -> None:
        ...
        try:
            ...
            call_blocking(functools.partial(tmp.unlink, missing_ok=True))
            raise
        finally:
            stream.close()
        call_blocking(tmp.replace, target)
    ...
    def assemble_evidence(
        ...
        *,
        ...
        result = self._post("/evidence", body)
