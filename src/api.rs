...
pub use consolidation::audit_consolidation;
pub use contexts::{
...
pub use import::{
    ...
    export_context, export_group, import_batch,
};
// `import_outcome`/`schema_import_outcome` are `pub(crate)` (import
// --json, issue #371/#384, is the only crate-internal reuse of a
...
mod tests {
    ...
    async fn restore_refusal_frames_a_spent_budget_as_a_resumable_timeout() {
        ...
        assert!(
            message.contains("group restore exceeded its budget with 2 source(s) durable"),
            "{message}"
        );
