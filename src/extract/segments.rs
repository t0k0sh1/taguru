//! Segment discovery and chunking: expanding CLI paths into files,
//! reading a segment's text, and splitting it into `ChunkDescriptor`
//! pieces at paragraph boundaries.

use super::*;

/// One chunk, with the paragraph-index provenance a diagnostics
/// consumer needs to point back at the original segment — issue
...
pub(crate) struct ChunkDescriptor {
    ...
    /// works on [`labeled_segment`]'s derived, relabeled rendering, so
...
pub(super) fn chunk_plan_preferring(
    ...
    chunk_preferring(&labeled_segment(text, cap), cap, &break_before)
        .into_iter()
        .map(|piece| {
            // Every block in a chunk is `[N] `-labeled and no block is
            // ever re-split by chunk() (labeled_segment reserves the
...
/// Parses the `[N] ` label [`labeled_segment`] prefixes onto every
/// block. A block without one would mean `chunk()`/`labeled_segment`
...
pub(super) fn leading_paragraph_number(block: &str) -> u32 {
    block
        ...
        .expect("labeled_segment prefixes every block with its [N] label")
}

/// The segment's text, refused early when it could never ride as a
...
/// an oversized segment (a mispointed path, a multi-GB log file) is
...
///
/// `pub(crate)` so `benchmark`'s preflight hashes a segment's text the
/// exact same way `extract`'s own manifest does (BOM-stripped, size-
/// capped, UTF-8-validated) — the segment dictionary in
...
pub(crate) fn read_segment(path: &Path) -> Result<String, String> {
    ...
    if bytes.len() as u64 > MAX_PASSAGE_BYTES as u64 {
        return Err(format!(
            "exceeds the {MAX_PASSAGE_BYTES}-byte segment cap — split the segment"
        ));
    }
    ...
    // match against the segment's true opening text.
...
pub(crate) fn expand_segments(paths: &[String]) -> Result<Vec<PathBuf>, String> {
