...
| GET | `/groups` | `?limit=1000&after=id` → `{total, groups:[{id, description, contexts, groups, fingerprint}]}` (`id` is the group's name — the row's own key, as its `group` record on file says; keyset paging by id; a group bundles contexts many-to-many and may nest child groups — `groups` — at most 3 tall, cycles refused; `fingerprint` = one change token over the transitive member contexts' `revision` counters — it moves exactly when a member you can see changed: a write, an embedding refresh, a rename, or a membership edit — same equality-only, re-check-after-restart contract as `revision`) |
| GET | `/groups/{name}` | one group row / 404 |
| PUT | `/groups/{name}` | `{description?, contexts?:[name], groups?:[name]}` → create (a group and a context may share the same name without conflict; every listed member — context or child group — must exist) |
...
| GET | `/contexts/{name}/communities` | community detection on the live graph (JSON Lines body, not the JSON envelope): a header line `{type:"communities", version, context, algorithm, revision, concept_count, edge_count, levels, communities}` — `revision` is the snapshot the analysis was cut at — then one line per community `{id, level, parent?, fingerprint, concept_count, members?:[{name, strength}], children?, top_associations?}`, leaves (level 0) first. Compute-heavy (heavy-ops gated); this is the derivation half `taguru communities` orchestrates — most callers want `communities/search` below instead |
