...
| POST | `/contexts/{name}/query` | `{subject?, label?, object?, subject_types?, object_types?, limit?, after?}` — each position a string or an array → `{total, matches, plan}` (`subject_types`/`object_types` filter by declared entity type, `is_a`-expanded, when the context has an installed schema — a filter, never an anchor, applied after the position pins and before paging, so `total`/`after` see the post-filter count; a schema-free context answers empty for a non-empty filter) |
