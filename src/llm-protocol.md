...
- Keys may carry a scope (`TAGURU_KEY_SCOPES`): a role — read (the
  ...
  content), and a group write touching any context beyond the grant —
  counted through nested children — is refused whole. Scopes bind MCP
  tool calls exactly as raw HTTP.
